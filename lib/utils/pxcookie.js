...
function isCookieValid(cookie, cookieKey, ip, ua) {
    try {
        ...
        console.error('Error while validating perimeterx cookie', e.stack);
        return false;
    }
}
...
/**
 * evalCookie - main cookie evaluation function. decrypt, decode and verify cookie content.
 *
 * @param {String} cookie - cookie raw value.
 * @param {String} ip - user ip, used to validate cookie signing.
 * @param {String} ua - user's useragent, also used to validate cookie signing.
 * @param {Object} pxCtx - current request context.
 *
 * @return {Number} evaluation result, derived from configured enum on PX_DEFAULT.COOKIE_EVAL. possible values: (NO_COOKIE, COOKIE_INVALID, COOKIE_EXPIRED, UNEXPECTED_RESULT, BAD_SCORE, GOOD_SCORE).
 *
 */
function evalCookie(cookie, ip, ua, pxCtx) {
    if (!cookie) {
        pxlogger.debug('No cookie found, pausing cookie evaluation');
        return pxconfig.SCORE_EVALUATE_ACTION.NO_COOKIE;
    }
    const cookieKey = pxconfig.COOKIE_SECRET_KEY;
    if (!cookieKey) {
        pxlogger.debug('No cookie key found, pausing cookie evaluation');
